...
import { investigateFeature } from './investigator'
...
Use this when asked about: recent work, what was shipped/pushed/committed recently, activity today/this week/this month, what's being worked on right now, latest contributions.`,
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // 2. Feature Implementation Investigator
    // ─────────────────────────────────────────────────────────────────────────
    investigateFeatureImplementation: tool({
      description: `Investigates how a specific feature is implemented in a repository.
Use this FIRST for questions like: "how did you build X?", "how is Y implemented?", "walk me through the map view/auth flow/search pipeline".
It performs multi-step discovery (repo tree scan + code search + candidate file inspection) and returns concrete implementation evidence.
Do not conclude "not found" until this tool has been used.`,
      parameters: z.object({
        repo: z.string().describe('Repository name'),
        feature: z.string().describe('Feature to investigate, e.g. "map view", "auth flow", "checkout"'),
        maxFiles: z
          .number()
          .default(6)
          .describe('How many files to inspect in detail (default 6, max 10)'),
      }),
      execute: async ({ repo, feature, maxFiles }) => {
        return investigateFeature(client, login, repo, feature, maxFiles)
      },
    }),
